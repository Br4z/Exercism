...
triangle::flavor triangle::kind(double a, double b, double c) {
	...
	if (!are_all_sides_positive) throw std::domain_error("Triangles with negative sides are illegal");
	else if (!are_all_inequalities_satisfied) throw std::domain_error("Triangles violating triangle inequality are illegal");
	else {
		if (a == b && b == c) return triangle::flavor::equilateral;
		else if (a == b || b == c || a == c) return triangle::flavor::isosceles;
		else return triangle::flavor::scalene;
	}
}
